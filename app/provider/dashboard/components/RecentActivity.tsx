...
import { Activity, FileText, MessageSquare, CheckCircle, CreditCard, RotateCcw, UserPlus, Truck, AlertTriangle, Sparkles } from 'lucide-react'
import { formatRelativeTime } from '@/lib/format'
import { getEventStyle, type EventStyle } from '@/lib/status-config'
import type { ProviderActivity } from '../../_lib/server-data'
...
export function RecentActivity({ activities }: RecentActivityProps) {
	return (
		<div className='min-w-0 overflow-hidden rounded-xl border border-slate-200 bg-white p-3 shadow-sm'>
			...
			{activities.length === 0 ? (
				<p className='flex items-center justify-center py-8 text-[12px] text-slate-400'>
					No recent activity
				</p>
			) : (
				<div className='space-y-1.5'>
					{activities.map((item) => {
						...
						return (
							<div
								key={item.id}
								className='flex min-w-0 items-start gap-2.5 rounded-lg bg-slate-50 px-2.5 py-2'
							>
								...
								<div className='min-w-0 flex-1'>
									...
									{item.title && item.requestId ? (
										<Link
											...
											className='truncate text-[11px] text-slate-400 hover:text-slate-600 block'
										>
											{item.title}
										</Link>
									) : item.title ? (
