...
import type { Attachment, SupportFile, SupportMessage, SupportThread } from '@/types/support'
import {
...
export function ThreadView({ thread, onRefresh, compact = false, onOpenMilestones }: ThreadViewProps) {
	...
	const uploadPendingFiles = async () => {
		...
		const res = await fetch('/api/support/files/upload', { method: 'POST', body: fd })
	...
	const handleSend = async () => {
		...
		if ((!body && pendingFiles.length === 0) || sendMessage.isPending || uploading) return
		setUploading(true)
		try {
			...
			const attachments = uploadedFiles.map((file) => ({
				...
				name: file.fileName,
				label: file.fileName,
				url: file.fileUrl || `/api/support/files/${file.id}/download`,
	...
	const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		toast.success(files.length === 1 ? 'File attached. Hit Send to share it.' : `${files.length} files attached. Hit Send to share them.`)
	...
	const handleFinalDelivery = async () => {
		...
		try {
			...
			for (const image of previewImages) fd.append('previewImages', image, image.name)
			fd.append('deliveryNote', 'Preview page images are available in chat; clean final files stay locked until full payment.')
			const res = await fetch(`/api/support/provider/requests/${thread.requestId}/deliver`, {
				method: 'POST',
				body: fd,
			})
			const payload = await res.json().catch(() => ({}))
			if (!res.ok) throw new Error(payload?.error || 'Delivery failed')
			toast.success('Preview pages published. Clean final files remain locked until full payment.')
			onRefresh?.()
			setDeliveryPdf(null)
			setDeliveryDocx(null)
			setPreviewImages([])
		} catch (error) {
			toast.error(error instanceof Error ? error.message : 'Failed to upload final delivery')
		} finally {
			setDelivering(false)
			...
			if (previewImagesInputRef.current) previewImagesInputRef.current.value = ''
		}
	}
	...
	const deliveryPackagePanel = (
		<div className='space-y-3'>
			...
			<div className='min-w-0'>
				<p className='text-xs font-semibold text-slate-900'>Publish delivery package</p>
				<p className='text-[11px] leading-4 text-slate-500'>
					Upload clean files and ordered preview images. Clean files stay locked until payment.
				</p>
			</div>
			<div className='grid min-w-0 grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-[1fr_1fr_1.15fr_auto]'>
				...
				>
					...
					<span className='truncate'>{deliveryDocx?.name || 'Select DOCX'}</span>
				</Button>
				<Button
					type='button'
					variant='outline'
				...
				>
					...
					<span className='truncate'>{previewImages.length ? `${previewImages.length} preview images` : 'Select preview images'}</span>
				</Button>
				<Button
					type='button'
					onClick={() => void handleFinalDelivery()}
					disabled={delivering || !deliveryPdf || !deliveryDocx || previewImages.length === 0}
			...
			{previewImages.length > 0 && (
				<div className='flex gap-2 overflow-x-auto rounded-lg border border-slate-200 bg-white p-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden'>
					{previewImages.map((file, index) => (
						<div key={`${file.name}-${file.size}-${index}`} className='flex min-w-[190px] max-w-[240px] items-center gap-2 rounded-lg bg-slate-50 px-2 py-1.5 text-[11px] text-slate-600'>
							...
							<div className='ml-auto flex shrink-0 items-center gap-1'>
								<button type='button' aria-label={`Move preview image ${index + 1} earlier`} disabled={index === 0 || delivering} onClick={() => movePreviewImage(index, -1)} className='flex h-6 w-6 items-center justify-center rounded-md text-slate-400 hover:bg-slate-200 hover:text-slate-700 disabled:cursor-not-allowed disabled:opacity-35'>
									<ArrowUp className='h-3.5 w-3.5' />
								</button>
								<button type='button' aria-label={`Move preview image ${index + 1} later`} disabled={index === previewImages.length - 1 || delivering} onClick={() => movePreviewImage(index, 1)} className='flex h-6 w-6 items-center justify-center rounded-md text-slate-400 hover:bg-slate-200 hover:text-slate-700 disabled:cursor-not-allowed disabled:opacity-35'>
									<ArrowDown className='h-3.5 w-3.5' />
								</button>
							</div>
							<button type='button' aria-label={`Remove preview image ${index + 1}`} onClick={() => setPreviewImages((current) => current.filter((_, itemIndex) => itemIndex !== index))} className='flex h-6 w-6 shrink-0 items-center justify-center rounded-md text-slate-400 hover:bg-slate-200 hover:text-slate-700'>
	...
	return (
		<div className={cn('relative flex flex-col overflow-hidden', compact ? 'h-full min-h-0' : 'min-h-[680px]')}>
			{/* Non-compact header */}
			{!compact && (
				<div className='shrink-0 border-b border-slate-100 bg-white px-4 py-2.5 flex items-center justify-between'>
					<div>
						<p className='text-[13px] font-semibold text-slate-900'>{thread.requestTitle || 'Support Chat'}</p>
						<p className='text-[11px] text-slate-400'>
							{realtime.status === 'connected' ? 'Live' : realtime.status === 'connecting' ? 'Connecting…' : 'Offline'}
						</p>
					</div>
				</div>
			)}
					...
					<div className='lg:hidden'>
						...
						>
							...
							<span className='text-[11px] text-slate-500'>
								{[deliveryPdf, deliveryDocx, previewImages.length ? previewImages : null].filter(Boolean).length}/3 ready
							</span>
						</Button>
					</div>
			...
			>
				{isLoading ? (
					...
					<div className='flex min-h-[200px] items-center justify-center text-[12px] text-slate-400'>No messages yet</div>
				) : (
					messages.map((msg) => (
						<MessageBubble
							key={msg.id}
							message={msg}
							isOwnMessage={msg.canEdit ?? (msg.senderKeyId === session?.user?.id || msg.senderId === session?.user?.id)}
							handleEditMessage={handleEditMessage}
							handleDeleteMessage={async (messageId) => setDeleteTargetId(messageId)}
			...
			<div className='pointer-events-none absolute inset-x-0 bottom-0 z-10 bg-gradient-to-t from-white via-white/95 to-transparent px-2.5 pb-[calc(0.375rem+env(safe-area-inset-bottom,0px))] pt-3 sm:px-3 sm:pb-[calc(0.5rem+env(safe-area-inset-bottom,0px))] sm:pt-4'>
				...
				{thread.requestId && (
					<div className='pointer-events-auto mb-1.5 grid grid-cols-1 gap-1 sm:mb-2 lg:hidden'>
						<QuickBtn icon={MoreHorizontal} label='Open chat tools' color='text-slate-600' onClick={() => setShowCardTools(true)} />
					</div>
				)}

				{thread.requestId && (
					<div className='pointer-events-auto mb-1.5 hidden grid-cols-4 gap-1 sm:mb-2 sm:gap-1.5 lg:grid'>
						<QuickBtn icon={ClipboardCheck} label='Milestone card' color='text-blue-600' onClick={handleMilestoneTool} />
					...
					<div className='pointer-events-auto mb-1.5 flex max-h-16 flex-wrap gap-1 overflow-y-auto rounded-lg border border-emerald-100 bg-emerald-50 p-1.5'>
						{pendingFiles.map((file, index) => (
							<span
								...
								className='flex min-w-0 max-w-full items-center gap-1 rounded-md bg-white px-2 py-1 text-[11px] font-medium text-emerald-800 shadow-sm'
							>
				...
				{replyingTo && (
					<div className='pointer-events-auto mb-1.5 flex items-center justify-between gap-2 rounded-lg border border-blue-100 bg-blue-50 p-2'>
						<div className='min-w-0 flex-1'>
							<p className='text-[10px] font-semibold text-blue-700'>Replying to {replyingTo.senderName}</p>
							<p className='truncate text-[11px] text-blue-600'>{replyingTo.preview}</p>
						</div>
						<button
							type='button'
				...
				{/* Textarea + send */}
				<div className='pointer-events-auto flex items-end gap-1.5 rounded-xl border border-slate-200 bg-white p-1.5 shadow-sm'>
					<Textarea
						ref={textareaRef}
						value={newMessage}
					...
					<div className='flex flex-col gap-1 pb-0.5'>
						...
						<button
							type='button'
							onClick={handleSend}
							disabled={sendMessage.isPending || uploading || (!newMessage.trim() && pendingFiles.length === 0)}
							title='Send'
						...
						>
							{sendMessage.isPending || uploading ? <Loader2 className='h-3.5 w-3.5 animate-spin' /> : <Send className='h-3.5 w-3.5' />}
						</button>
					</div>
				</div>
			</div>
			...
			<Drawer open={showCardTools} onOpenChange={setShowCardTools}>
				<DrawerContent className='rounded-t-2xl'>
					...
					<div className='grid grid-cols-2 gap-2 px-4 pb-[calc(1rem+env(safe-area-inset-bottom,0px))]'>
						<QuickBtn icon={ClipboardCheck} label='Milestone card' color='text-blue-600' onClick={() => { setShowCardTools(false); handleMilestoneTool() }} />
						<PaymentCardBtn requestId={thread.requestId ?? ''} onSent={onRefresh} />
						<RevisionCardBtn requestId={thread.requestId ?? ''} onSent={onRefresh} />
						<DeliveryCardBtn requestId={thread.requestId ?? ''} onSent={onRefresh} />
					</div>
...
function ProviderConfirmDialog({
	...
	return (
		<Dialog open={open} onOpenChange={handleOpenChange}>
			<DialogContent fallbackTitle={title} showCloseButton={!isLoading} className='sm:max-w-[420px] sm:rounded-2xl sm:p-0'>
				<div className='px-1 pb-[calc(0.5rem+env(safe-area-inset-bottom,0px))] pt-1 sm:px-0 sm:pb-0 sm:pt-0'>
					<DialogHeader className='gap-4 border-b border-slate-200 px-1 pb-5 text-left sm:px-6 sm:pt-6'>
...
function QuickBtn({
	...
	return (
		<button
			type='button'
			onClick={onClick}
			className='flex h-8 items-center justify-center gap-1.5 rounded-lg border border-slate-200 bg-white px-2 text-[11px] font-medium text-slate-700 transition hover:bg-slate-50 hover:shadow-sm'
		>
...
function PaymentCardBtn({ requestId, onSent }: { requestId: string; onSent?: () => void }) {
	...
	const loadAmounts = useCallback(async () => {
		try {
			...
			const payload = await res.json().catch(() => ({}))
			if (!res.ok) throw new Error(payload?.error || 'Failed to load payment amounts')
			const request = payload?.data || {}
			const full = Number(request.finalAmount ?? request.paymentAmount ?? request.quotedAmount ?? 0)
			...
			toast.error(error instanceof Error ? error.message : 'Failed to load payment amounts')
		}
	...
	const handlePaymentTypeChange = (nextPaymentType: string) => {
		...
		setAmount(String(requestAmounts[nextPaymentType as keyof typeof requestAmounts] || ''))
	}
	...
	const send = async () => {
		...
		setBusy(true)
		try {
			const res = await fetch(`/api/support/provider/requests/${requestId}/send-card`, {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ kind: 'payment_card', amount: Number(amount), paymentType, note }),
			})
			const payload = await res.json().catch(() => ({}))
			if (!res.ok) throw new Error(payload?.error || 'Failed to send payment card')
					...
					<div className='rounded-t-2xl bg-gradient-to-br from-emerald-600 to-teal-700 p-4 text-white sm:p-5'>
						<div className='mb-2.5 flex items-center gap-2.5 sm:mb-3'>
							...
							<div>
								...
								<p className='text-[11px] opacity-60'>From your service provider · CogniZap</p>
							</div>
						</div>
						<p className='text-[28px] font-bold leading-none tracking-tight sm:text-[32px]'>
							{amount ? `GHS ${Number(amount).toLocaleString()}` : 'GHS —'}
						</p>
						<p className='mt-1 text-[12px] opacity-75 capitalize'>{paymentType.replace('_', ' ')} payment</p>
						{note && <p className='mt-1 text-[11px] opacity-60 line-clamp-1'>{note}</p>}
						<div className='mt-3 hidden items-center gap-2 rounded-lg bg-white/10 px-3 py-2 xs:flex'>
							<div className='h-2 w-2 rounded-full bg-white/60' />
							<p className='text-[11px] opacity-80'>Client clicks "Pay Now" → Paystack opens</p>
						</div>
					</div>
					...
					<div className='space-y-2.5 p-3 sm:p-4'>
						<FormField label='Amount (GHS)'>
							<Input
								type='number'
								value={amount}
								onChange={(e) => paymentType === 'partial_balance' && setAmount(e.target.value)}
						...
						<div className='grid grid-cols-2 gap-2 pt-1'>
							<Button size='sm' onClick={send} disabled={busy || !amount} className='gap-1.5 bg-emerald-600 hover:bg-emerald-700'>
...
function RevisionCardBtn({ requestId, onSent }: { requestId: string; onSent?: () => void }) {
	const [open, setOpen] = useState(false)
	...
	const send = async () => {
		if (!message.trim()) { toast.error('Add a message for the client'); return }
		setBusy(true)
		try {
			const res = await fetch(`/api/support/provider/requests/${requestId}/send-card`, {
				method: 'POST',
			...
			const payload = await res.json().catch(() => ({}))
			if (!res.ok) throw new Error(payload?.error || 'Failed to send revision card')
			toast.success('Revision card sent to client')
			setTitle('Revision Update'); setMessage(''); setExpectedAt(''); setOpen(false)
			onSent?.()
		} catch (err) {
			toast.error(err instanceof Error ? err.message : 'Failed to send card')
		} finally {
				...
				<CardDialog onClose={() => setOpen(false)}>
					...
					<div className='rounded-t-2xl bg-gradient-to-br from-amber-500 to-orange-600 p-4 text-white sm:p-5'>
						<div className='mb-2.5 flex items-center gap-2.5 sm:mb-3'>
							...
							<div>
								<p className='text-[13px] font-bold'>{title || 'Revision Update'}</p>
								<p className='text-[11px] opacity-60'>From your provider · CogniZap</p>
							</div>
						</div>
						{message && <p className='text-[12px] leading-relaxed opacity-80 line-clamp-3'>{message}</p>}
						{expectedAt && (
							<div className='mt-3 flex items-center gap-1.5 rounded-lg bg-white/10 px-3 py-2'>
								<CalendarClock className='h-3.5 w-3.5 opacity-70' />
								<p className='text-[11px] opacity-80'>Expected: {new Date(expectedAt).toLocaleDateString()}</p>
							</div>
						)}
					</div>
					...
					<div className='space-y-2.5 p-3 sm:p-4'>
						...
						<FormField label='Expected completion (optional)'>
							<Input type='date' value={expectedAt} onChange={(e) => setExpectedAt(e.target.value)} className='h-8 text-[13px]' />
						</FormField>
						<div className='grid grid-cols-2 gap-2 pt-1'>
							<Button size='sm' onClick={send} disabled={busy || !message.trim()} className='gap-1.5 bg-amber-500 hover:bg-amber-600'>
								{busy ? <Loader2 className='h-3.5 w-3.5 animate-spin' /> : <Send className='h-3.5 w-3.5' />}
								Send card
							</Button>
							<Button size='sm' variant='outline' onClick={() => setOpen(false)}>Cancel</Button>
...
function DeliveryCardBtn({ requestId, onSent }: { requestId: string; onSent?: () => void }) {
	const [open, setOpen] = useState(false)
	...
	const send = async () => {
		...
		try {
			const res = await fetch(`/api/support/provider/requests/${requestId}/send-card`, {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ kind: 'delivery_card', title, message, locked: true }),
			})
			const payload = await res.json().catch(() => ({}))
			if (!res.ok) throw new Error(payload?.error || 'Failed to send delivery card')
			toast.success('Delivery card sent to client')
			setTitle('Work Delivered'); setMessage(''); setOpen(false)
			onSent?.()
		} catch (err) {
			toast.error(err instanceof Error ? err.message : 'Failed to send card')
		} finally {
				...
				<CardDialog onClose={() => setOpen(false)}>
					...
					<div className='rounded-t-2xl bg-gradient-to-br from-indigo-600 to-purple-700 p-4 text-white sm:p-5'>
						<div className='mb-2.5 flex items-center gap-2.5 sm:mb-3'>
							...
							<div>
								<p className='text-[13px] font-bold'>{title || 'Work Delivered'}</p>
								<p className='text-[11px] opacity-60'>From your provider · CogniZap</p>
							</div>
						</div>
						{message && <p className='text-[12px] leading-relaxed opacity-80 line-clamp-3'>{message}</p>}
						<div className='mt-3 hidden items-center gap-1.5 rounded-lg bg-white/10 px-3 py-2 xs:flex'>
							<CheckCircle className='h-3.5 w-3.5 opacity-70' />
							<p className='text-[11px] opacity-80'>Files are ready — complete payment to unlock</p>
						</div>
					</div>
					...
					<div className='space-y-2.5 p-3 sm:p-4'>
						...
						<div className='grid grid-cols-2 gap-2 pt-1'>
							<Button size='sm' onClick={send} disabled={busy} className='gap-1.5 bg-indigo-600 hover:bg-indigo-700'>
								{busy ? <Loader2 className='h-3.5 w-3.5 animate-spin' /> : <Send className='h-3.5 w-3.5' />}
								Send card
							</Button>
							<Button size='sm' variant='outline' onClick={() => setOpen(false)}>Cancel</Button>
...
function CardDialog({ children, onClose }: { children: React.ReactNode; onClose: () => void }) {
	return (
...
import { FilePreview } from '@/components/support/FilePreview'
...
function MessageBubble({ message: msg, isOwnMessage, handleEditMessage, handleDeleteMessage, setReplyingTo, onFilesChanged, onReplacePackage }: MessageBubbleProps) {
		...
		<div className={cn('flex', isProvider ? 'justify-end' : 'justify-start')}>
			...
			>
				...
				{msg.replyToMessage && !isDeleted && (
					<div className={cn('mb-2 rounded-lg px-2 py-1 text-[11px] border-l-2', isProvider ? 'bg-white/10 border-white/30' : 'bg-slate-200/50 border-slate-400')}>
						<p className={cn('font-semibold', isProvider ? 'text-white/70' : 'text-slate-600')}>
							Replying to {msg.replyToMessage.senderRole === 'provider' ? 'Provider' : 'Client'}
						</p>
						<p className={cn('truncate', isProvider ? 'text-white/60' : 'text-slate-500')}>
				...
				{isDeleted ? (
					...
					<div className='space-y-2'>
						<Textarea
							...
							className='min-h-20 resize-none border-0 bg-white/10 text-white placeholder:text-white/50'
							autoFocus
						/>
						<div className='flex gap-1.5'>
							<Button size='sm' onClick={handleSaveEdit} className='h-7 gap-1 bg-white text-emerald-600 hover:bg-white/90'>
								<Pencil className='h-3 w-3' />
								Save
							</Button>
							<Button size='sm' variant='outline' onClick={handleCancelEdit} className='h-7 gap-1 bg-white/10 border-white/20 text-white hover:bg-white/20'>
								<X className='h-3 w-3' />
								Cancel
							</Button>
						</div>
					...
					<div className='mt-2 space-y-1.5'>
						{attachments.map((att, idx) => {
							...
							if (att.kind === 'file_event') {
								return <FileActivityCard key={key} attachment={att} isProvider={isProvider} />
							}
					...
					<div className='mt-2 flex items-center gap-1'>
						<button
							...
							className={cn(
								...
								isProvider ? 'text-white/70 hover:bg-white/10 hover:text-white' : 'text-slate-500 hover:bg-slate-200 hover:text-slate-900',
							)}
							title='Reply'
						>
				...
				<p className={cn('mt-1 text-[10px]', isDeleted ? 'text-slate-400' : isProvider ? 'text-white/60' : 'text-slate-400')}>
...
function ProviderManagedFileAttachment({
	...
	const fileName = attachment.name || attachment.label || 'Attachment'
	const deleted = attachment.status === 'deleted' || Boolean(attachment.deletedAt)
	...
	const handleReplace = async (file: File | undefined) => {
		...
		try {
			...
			const response = await fetch(`/api/support/files/${fileId}`, {
				method: 'PATCH',
				body: form,
			})
			const payload = await response.json().catch(() => ({}))
			if (!response.ok) throw new Error(payload?.error || 'Could not replace file')
			toast.success('File replaced and noted in chat')
			onChanged?.()
		} catch (error) {
			toast.error(error instanceof Error ? error.message : 'Could not replace file')
		} finally {
			setReplacing(false)
	...
	const handleDelete = async () => {
		...
		try {
			...
			const payload = await response.json().catch(() => ({}))
			if (!response.ok) throw new Error(payload?.error || 'Could not delete file')
	...
	if (deleted) {
		return (
			...
			>
				...
				<p className='mt-0.5 text-[10px]'>This file was deleted by the provider.</p>
			</div>
		)
	}
		...
		<div className='overflow-hidden rounded-lg'>
			...
			<div
				className={cn(
					...
					isProvider ? 'bg-white/10' : 'bg-slate-50',
				)}
			>
				{edited && (
					<span className={cn('mr-auto text-[10px]', isProvider ? 'text-white/65' : 'text-slate-500')}>
						Edited
				...
				<Button
					...
					className={cn(
						...
						isProvider ? 'text-white/80 hover:bg-white/10 hover:text-white' : 'text-slate-600',
					)}
				>
				...
				<Button
					...
					className={cn(
						'h-7 gap-1 px-2 text-[10px]',
						isProvider ? 'text-white/80 hover:bg-white/10 hover:text-white' : 'text-red-600',
					)}
				>
...
function MilestoneChatCard({ attachment }: { attachment: NonNullable<SupportMessage['attachments']>[number] }) {
	const status = String(attachment.status ?? 'submitted')
	const revisionPending = status === 'revision_requested'
	const revisionMessage = attachment.latestRevisionMessage || attachment.userFeedback
	const files = Array.isArray(attachment.files) ? attachment.files : []

	return (
		<div className={cn('mt-1.5 rounded-xl border bg-white p-3 text-slate-900 shadow-sm', revisionPending ? 'border-amber-200' : 'border-slate-200')}>
			<div className='flex items-start justify-between gap-2'>
				<div className='min-w-0'>
					...
					<p className='mt-0.5 truncate text-[13px] font-semibold'>{attachment.title || 'Milestone'}</p>
					{attachment.description && <p className='mt-0.5 line-clamp-1 text-[11px] text-slate-500'>{attachment.description}</p>}
				</div>
				<span className={cn('shrink-0 rounded-full px-2 py-0.5 text-[10px] font-semibold', revisionPending ? 'bg-amber-50 text-amber-700' : 'bg-blue-50 text-blue-700')}>
					{fmtStatus(status)}
				</span>
			</div>
			<div className='mt-2 grid grid-cols-3 gap-1'>
				{[
					...
					{ label: revisionPending ? 'Revision' : 'In review', active: true, warn: revisionPending },
					{ label: status === 'approved' ? 'Approved' : 'Pending', active: status === 'approved' },
				].map((step) => (
					<div key={step.label}>
						<div className={cn('h-1 rounded-full', step.active ? (step.warn ? 'bg-amber-400' : 'bg-blue-500') : 'bg-slate-200')} />
						<p className='mt-0.5 truncate text-[10px] text-slate-400'>{step.label}</p>
					</div>
				))}
			</div>
			<div className='mt-2 flex flex-wrap gap-x-3 gap-y-1 text-[11px] text-slate-400'>
				<span className='flex items-center gap-1'><CalendarClock className='h-3 w-3' />{fmtDate(attachment.dueAt)}</span>
			...
			{files.length > 0 && (
				<div className='mt-2 grid gap-1.5 rounded-lg border border-slate-200 bg-slate-50 p-2'>
					{files.map((file, index) => {
						const fileName = file.name || file.label || file.title || `Milestone file ${index + 1}`
						...
						return (
							<div key={file.id || file.fileId || `${fileName}-${index}`} className='flex min-w-0 items-center gap-2 rounded-lg bg-white px-2 py-1.5 text-[11px] text-slate-600'>
								{locked ? <ClipboardCheck className='h-3.5 w-3.5 shrink-0 text-amber-600' /> : <Paperclip className='h-3.5 w-3.5 shrink-0 text-blue-600' />}
								<span className={cn('truncate font-medium', deleted && 'line-through')}>{fileName}</span>
								<span className={cn('ml-auto shrink-0 rounded-full px-1.5 py-0.5 text-[9px] font-semibold', deleted ? 'bg-red-50 text-red-700' : locked ? 'bg-amber-50 text-amber-700' : edited ? 'bg-blue-50 text-blue-700' : 'bg-emerald-50 text-emerald-700')}>
									{deleted ? 'Deleted' : locked ? 'Locked' : edited ? 'Edited' : 'Ready'}
								</span>
							</div>
						)
					})}
			...
			{revisionMessage && (
				<div className='mt-2 rounded-lg border border-amber-200 bg-amber-50 p-2 text-[11px] text-amber-800'>
					<p className='font-semibold'>{attachment.latestRevisionReason || 'Revision note'}</p>
...
function PaymentChatCard({ attachment }: { attachment: NonNullable<SupportMessage['attachments']>[number] }) {
	...
	const paymentLabel =
		paymentType === 'deposit' ? 'Deposit payment'
			: paymentType === 'final_balance' ? 'Balance payment'
				: paymentType === 'partial_balance' ? 'Milestone payment'
					: paymentType.replace(/_/g, ' ')

	return (
		<div className='mt-1.5 overflow-hidden rounded-xl border border-emerald-200 bg-white shadow-sm'>
			<div className='bg-gradient-to-br from-emerald-600 to-teal-700 px-3 py-3 text-white'>
				<div className='flex items-center gap-2 mb-1.5'>
					...
					<p className='text-[11px] font-semibold opacity-80'>Payment Request</p>
				</div>
				<p className='text-[22px] font-bold tracking-tight'>{currency} {amt.toLocaleString()}</p>
				<p className='text-[11px] opacity-70 capitalize mt-0.5'>{paymentLabel}</p>
				{note && <p className='text-[11px] opacity-60 mt-0.5 line-clamp-1'>{note}</p>}
			</div>
...
function RevisionChatCard({ attachment }: { attachment: NonNullable<SupportMessage['attachments']>[number] }) {
	...
	return (
		<div className='mt-1.5 overflow-hidden rounded-xl border border-amber-200 bg-white shadow-sm'>
			<div className='bg-gradient-to-br from-amber-500 to-orange-600 px-3 py-3 text-white'>
				<div className='flex items-center gap-2 mb-1.5'>
					<RotateCcw className='h-4 w-4 opacity-80' />
					<p className='text-[11px] font-semibold opacity-80'>Revision Update</p>
				</div>
				<p className='text-[14px] font-bold'>{title}</p>
				{message && <p className='text-[11px] opacity-75 mt-1 line-clamp-2 leading-relaxed'>{message}</p>}
			</div>
			{expectedAt && (
				<div className='flex items-center gap-2 border-t border-amber-100 px-3 py-2 text-[11px] text-slate-500'>
...
function DeliveryChatCard({ attachment }: { attachment: NonNullable<SupportMessage['attachments']>[number] }) {
	const title = String(attachment.title ?? 'Work Delivered')
	const message = String(attachment.message ?? attachment.description ?? '')
	const locked = attachment.locked !== false

	return (
		<div className='mt-1.5 overflow-hidden rounded-xl border border-indigo-200 bg-white shadow-sm'>
			...
			<div className='flex items-center gap-2 border-t border-indigo-100 px-3 py-2 text-[11px] text-slate-500'>
				<CheckCircle className='h-3.5 w-3.5 text-indigo-500' />
				{locked ? 'Files locked — awaiting payment' : 'Files unlocked for download'}
			</div>
		</div>
	)
}
...
function PreviewPagesChatCard({
	...
	const locked = packageAttachment.locked !== false || packageAttachment.cleanFilesLocked !== false
	const files: Array<Record<string, unknown>> = Array.isArray(packageAttachment.files) && packageAttachment.files.length > 0
		? packageAttachment.files
		: [
			{ name: 'Clean PDF file', fileName: 'Clean PDF file', fileType: 'application/pdf', locked: true },
			{ name: 'Editable DOCX file', fileName: 'Editable DOCX file', fileType: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', locked: true },
		]
	return (
		<div className='mt-1.5 overflow-hidden rounded-xl border border-emerald-200 bg-white shadow-sm'>
			<div className='bg-emerald-50 px-3 py-3 text-slate-900'>
				<div className='mb-1.5 flex items-center gap-2'>
					<Images className='h-4 w-4 text-emerald-600' />
					<p className='text-[11px] font-semibold text-emerald-700'>Preview Pages Published</p>
				</div>
				<p className='text-[14px] font-bold'>{attachment.title || 'Preview pages ready'}</p>
				<p className='mt-1 text-[11px] leading-relaxed text-slate-500'>
					{pageCount} page{pageCount === 1 ? '' : 's'} are visible to the client in chat. Clean PDF and DOCX stay locked until payment.
				</p>
			</div>
			<div className='space-y-2 border-t border-emerald-100 bg-white p-2.5'>
				{files.slice(0, 2).map((file, index) => {
					const item = file
					const fileName = String(item.fileName ?? item.name ?? item.label ?? (index === 0 ? 'Clean PDF file' : 'Editable DOCX file'))
					...
					return (
						<div key={`${fileName}-${index}`} className='flex min-w-0 items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 p-2 text-slate-900'>
							<div className={cn('flex h-9 w-9 shrink-0 items-center justify-center rounded-lg', isPdf ? 'bg-red-50 text-red-600' : 'bg-blue-50 text-blue-600')}>
								<FileText className='h-4 w-4' />
							</div>
							<div className='min-w-0 flex-1'>
								<p className='truncate text-[12px] font-semibold'>{fileName}</p>
								<p className='text-[10px] text-slate-500'>
									{isPdf ? 'PDF' : 'DOCX'}{rawSize > 0 ? ` - ${formatFileSize(rawSize)}` : ''} - {locked ? 'Locked for client' : 'Unlocked'}
								</p>
							</div>
							{locked ? <ClipboardCheck className='h-4 w-4 shrink-0 text-amber-600' /> : <CheckCircle className='h-4 w-4 shrink-0 text-emerald-600' />}
						</div>
					)
				})}
				<div className='grid grid-cols-2 gap-1.5'>
					<Button type='button' size='sm' variant='outline' onClick={onReplacePackage} className='h-9 gap-1 text-[11px]'>
						<Upload className='h-3.5 w-3.5' />
						Replace package
					</Button>
					<Button type='button' size='sm' variant='outline' disabled className='h-9 gap-1 text-[11px]'>
