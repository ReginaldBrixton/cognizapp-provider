...
export function ViewToggle({ viewMode, onChange, storageKey, className }: ViewToggleProps) {
	const [mounted, setMounted] = useState(false)

	useEffect(() => {
		setMounted(true)
		const saved = localStorage.getItem(`${STORAGE_PREFIX}${storageKey}`)
		if (saved && (saved === 'list' || saved === 'split' || saved === 'detail')) {
	...
	return (
		<div className={cn('flex items-center gap-1 rounded-lg border border-slate-200 p-1 dark:border-slate-800', className)}>
			<Button
				type='button'
