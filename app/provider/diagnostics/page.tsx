...
export default function ProviderDiagnosticsPage() {
	...
	const runPaymentCard = async () => {
		...
		try {
			await callJson(
				'Send payment card',
				`/api/provider/requests/${requestId}/send-card`,
				{
					method: 'POST',
					body: JSON.stringify({
						kind: 'payment',
	...
	const runDiscountCode = async () => {
		setBusy('code')
		try {
			await callJson('Create discount code', '/api/provider/discount-codes', {
				method: 'POST',
				body: JSON.stringify({
					label: discountLabel,
			...
			await callJson(
				...
				`/api/provider/requests/${requestId}/discount-decision`,
				{
					...
					body: JSON.stringify({
						...
						approvedAmount:
							Math.round(((baseAmount * percent) / 100) * 100) / 100,
		...
		try {
			...
			if (file) {
				...
				if (!uploadResponse.ok)
					throw new Error(getPayloadError(uploadBody, 'File upload failed'))
				...
				`/api/messages/threads/${activeThreadId}/messages`,
				{
					...
					body: JSON.stringify({
						...
						attachments: uploaded.map((item) => ({
							...
							name: item.fileName,
							label: item.fileName,
							url: item.fileUrl || `/api/files/${item.id}/download`,
	...
	return (
		<PageContainer
			...
			subtitle='Production-safe test console for cards, chat files, messages, and discounts'
		>
			<div className='grid gap-3 lg:grid-cols-[minmax(280px,360px)_1fr]'>
				<section className='rounded-xl border border-slate-200 dark:border-border bg-white dark:bg-card p-3 shadow-sm'>
					<div className='mb-3 flex items-center justify-between gap-2'>
						<p className='text-[12px] font-semibold uppercase tracking-wide text-slate-500 dark:text-muted-foreground'>
							Target request
						</p>
						<Button
							size='sm'
					...
					<p className='mt-2 text-[12px] text-slate-500 dark:text-muted-foreground'>
						...
						<span className='font-semibold text-slate-800 dark:text-foreground'>
							{formatMoney(baseAmount, selectedRequest?.currency)}
						</span>
					</p>
				</section>
				...
				<section className='grid gap-3 md:grid-cols-2'>
					<div className='rounded-xl border border-slate-200 dark:border-border bg-white dark:bg-card p-3 shadow-sm'>
						<div className='mb-2 flex items-center gap-2 text-[13px] font-semibold text-slate-900 dark:text-foreground'>
						...
						/>
						<Button
							onClick={runPaymentCard}
							disabled={!requestId || busy === 'card'}
					...
					<div className='rounded-xl border border-slate-200 dark:border-border bg-white dark:bg-card p-3 shadow-sm'>
						<div className='mb-2 flex items-center gap-2 text-[13px] font-semibold text-slate-900 dark:text-foreground'>
							<BadgePercent className='h-4 w-4 text-emerald-600' /> Discount
							test
						</div>
						...
						<div className='mt-2 grid grid-cols-2 gap-2'>
							...
							<Button
								onClick={runRequestDiscount}
								disabled={!requestId || busy === 'discount'}
								className='h-9 gap-1.5'
							>
								{busy === 'discount' ? (
									<Loader2 className='h-3.5 w-3.5 animate-spin' />
					...
					<div className='rounded-xl border border-slate-200 dark:border-border bg-white dark:bg-card p-3 shadow-sm md:col-span-2'>
						<div className='mb-2 flex items-center gap-2 text-[13px] font-semibold text-slate-900 dark:text-foreground'>
							<MessageSquare className='h-4 w-4 text-emerald-600' /> Message and
							file test
						</div>
						<Textarea
							value={message}
			...
			<section className='rounded-xl border border-slate-200 dark:border-border bg-white dark:bg-card p-3 shadow-sm'>
				<p className='mb-2 text-[12px] font-semibold uppercase tracking-wide text-slate-500 dark:text-muted-foreground'>
					Latest results
				</p>
				<div className='space-y-2'>
					{results.length === 0 ? (
						<p className='rounded-lg border border-dashed border-slate-200 dark:border-border py-6 text-center text-[12px] text-slate-400 dark:text-muted-foreground/70'>
						...
						results.map((result, index) => (
							<div
								...
								className='rounded-lg border border-slate-200 dark:border-border bg-slate-50 dark:bg-muted p-2'
							>
								...
								<pre className='mt-1 max-h-36 overflow-auto whitespace-pre-wrap rounded-md bg-white dark:bg-card p-2 text-[11px] text-slate-600 dark:text-muted-foreground'>
