...
export function DiscountCodeForm({ onCreate }: DiscountCodeFormProps) {
	...
	return (
		<div className='min-w-0 rounded-provider-card border border-slate-200 dark:border-border bg-white dark:bg-card p-2.5 shadow-sm lg:p-4'>
			<p className='mb-1.5 text-[11px] font-semibold uppercase tracking-wide text-slate-400 dark:text-muted-foreground/70 lg:mb-3'>
				New code
			</p>

			<div className='mb-2'>
				<label className='mb-1 block text-[11px] font-medium text-slate-500 dark:text-muted-foreground'>
					Label
				</label>
				<Input
					value={label}
			...
			<div className='mb-2 grid min-w-0 grid-cols-[minmax(0,0.8fr)_minmax(0,0.62fr)_minmax(0,1fr)] gap-1.5 sm:gap-2 lg:mb-3'>
				<div>
					<label className='mb-1 flex items-center gap-1 truncate text-[10px] font-medium text-slate-500 dark:text-muted-foreground sm:text-[11px]'>
					...
					<div className='relative'>
						...
						/>
						<span className='absolute right-2.5 top-1/2 -translate-y-1/2 text-[11px] text-slate-400 dark:text-muted-foreground/70'>
							%
						</span>
					</div>
				</div>
