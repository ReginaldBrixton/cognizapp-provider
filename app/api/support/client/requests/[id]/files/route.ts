...
export async function GET(_request: NextRequest, { params }: RouteParams) {
	try {
		...
		const response = await fetch(
			`${BACKEND_URL}/api/support/client/requests/${id}/files`,
			{
				method: 'GET',
				headers,
				cache: 'no-store',
		...
		console.error('[Support Request Files API] GET error:', error)
		return NextResponse.json(
			{ success: false, error: 'Failed to fetch request files' },
			{ status: 500 },
		)
	}
}
